    ...
    def board_error(self):
        ...
        for i in range(len(self.board)):
            for j in range(len(self.board[i]) - 1, -1, -1):
                if self.possession[i][j] == "w":
                    print(
                        f"{bcolors.WARNING}{self.stable_board[i][j]}{bcolors.ENDC}",
                        end=" ",
                    )
                elif self.possession[i][j] == "b":
                    print(
                        f"{bcolors.OKBLUE}{self.stable_board[i][j]}{bcolors.ENDC}",
                        end=" ",
                    )
                else:
                    print(
                        f"{bcolors.OKGREEN}{self.stable_board[i][j]}{bcolors.ENDC}",
                        end=" ",
                    )
            print()
        ...
        if board_map != current_board_map:
            ...
            for i in range(len(current_board_map)):
                for j in range(len(current_board_map[i])):
                    ...
                    prev_on_board = (
                        True if self.board[i][j] in possible_pieces else False
                    )
                    ...
                    if piece_found and prev_on_board:
                        ...
                        old_position = (
                            "("
                            + str(self.picked_up_from["row"])
                            + ","
                            + str(self.picked_up_from["col"])
                            + ")"
                        )
                        ...
                        if old_position != new_position:
                            ...
                            if self.current_move % 1:
                                ...
                                if self.moving_piece == "p":
                                    output += self.board_cols[
                                        self.picked_up_from["col"]
                                    ]
                                    if (
                                        self.potential_captured["row"] == i
                                        and self.potential_captured["col"] == j
                                    ):
                                        output += "x"
                                    ...
                                    self.moving_piece == "K"
                                    and self.board_cols[self.picked_up_from["col"]]
                                    == "e"
                                ):
